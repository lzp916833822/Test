...
abstract class BaseDialogBuilder<T extends BaseDialogBuilder<T>> {
    ...
    public T setRequestBundle(Bundle bundle) {
        mBundle = bundle;
        return self();
    }
    ...
    /**
     * Like show() but allows the commit to be executed after an activity's
     * state is saved. This is dangerous because the commit can be lost if the
     * activity needs to later be restored from its state, so this should only
     * be used for cases where it is okay for the UI state to change
