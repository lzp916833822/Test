...
public class OverlayerView extends View
{
    ...
    private float wRate = 1.2f;
    private float hRate = 1.2f;
...
//		}
    ...
    public void setRate(float wRate, float hRate) {
		...
		/*
		if (width >= padding * 2) {
			pRect.left = (int)(padding * wRate / 2);
			pRect.right = (int)(width - (padding * wRate / 2));
		}
		if (height >= padding * 2) {
			pRect.top = (int)(padding * hRate / 2);
			pRect.bottom = (int)(height - (padding * hRate/2));
		}
